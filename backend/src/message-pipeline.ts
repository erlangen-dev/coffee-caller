...
class AggregatedCoffeeCall implements CoffeeCall {
  ...
  constructor(public messages: TimedCommand[]) {
    ...
    const participantsTmp = new Set<string>();
    for (const message of messages) {
      switch (message.type) {
        case 'start':
          this.status = 'inProgress';
          participantsTmp.add(message.name);
          break;
        case 'join':
          ...
          break;
        case 'leave':
          participantsTmp.delete(message.name);
          break;
        default:
    ...
    if (participantsTmp.size === 0) {
      this.status = 'cancelled';
    }

    this.participants = Array.from(participantsTmp.values());
  }
}
